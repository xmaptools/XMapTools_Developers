...
The bootstrap scripts in `Server/` are deployed at `https://xmaptools.ch/install.sh` (macOS) and `https://xmaptools.ch/install.ps1` (Windows).

**macOS** — for XMapTools 4.6 beta 1 the MATLAB Runtime has changed, so a full installation is required (use `--install-dev` instead of `--update-dev`):

```bash
curl -fsSL https://xmaptools.ch/install.sh | bash -s -- --install-dev
```

**Windows** — the runtime is unchanged; a simple update is sufficient:

```powershell
iex "& { $(irm https://xmaptools.ch/install.ps1) } --update-dev"
```

To see all available options and detected runtime versions, run the script with `--info`.
